...
def home(user_id):
    ...
    for result in results:
        ...
        result['total_volume'] = '$ ' + \
            "{:,.2f}".format(result['total_volume'])
        result['price_change_percentage_24h'] = "{:,.2f}".format(
        ...
        for i in range(0, 5000):
            if data['data'][i]['name'].lower() == coin_name:
                ...
                existing_trans = Transaction.query.filter_by(
                    user_id=user_id, coin_name=coin_name.capitalize()).first()
                if existing_trans:
                    # Update the existing transaction
                    existing_trans.amount_spent = float(
                        existing_trans.amount_spent) + float(amount_spent)
                    existing_trans.no_of_coins = float(
                        existing_trans.no_of_coins) + float(no_of_coins)
                    existing_trans.time_updated = datetime.now()
                    user = User.query.get(user_id)
                    portfolio_worth = float(
                        user.portfolio_worth) + (float(current_price) * float(no_of_coins))
                    session.commit()
                    session.close()
